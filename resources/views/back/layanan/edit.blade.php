         ...
         <form action="{{ url('layanan/' . $layanan->id) }}" method="post" enctype="multipart/form-data">
          ...
          <div class="mb-3">
              ...
              <input type="text" class="form-control @error('keterangan') is-invalid @enderror" name="keterangan" id="keterangan"
                placeholder="Masukan keterangan" value="{{ old('keterangan', $layanan->keterangan) }}" autofocus />
              @error('keterangan')
              <div class="invalid-feedback">{{ $message }}</div>
              @enderror
             </div>
                ...
                <input class="form-control" type="file" value="{{ old('gambar', $layanan->gambar) }}" name="gambar" id="gambar">
              </div>
